...
CODEC_ERROR PrepareCodebooks(ALLOCATOR *allocator, CODESET *cs)
{
	...
	RUNS_TABLE *runs_table = Alloc(allocator, runs_table_size);
	...
	mags_table = Alloc(allocator, mags_table_size);
	if (mags_table == NULL) {
...
{
	Free(allocator, (void *)cs->runs_table);
	Free(allocator, (void *)cs->mags_table);
	return CODEC_ERROR_OKAY;
}
									  ...
									  RLC *output_codes, int output_length)
{
	...
	RLC *runs_codebook = (RLC *)Alloc(allocator, runs_codebook_size);
