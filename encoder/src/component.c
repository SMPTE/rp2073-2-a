...
CODEC_ERROR AllocateComponentTransform(ALLOCATOR *allocator, COMPONENT_TRANSFORM *transform, int component_count)
{
    ...
    transform->transform_matrix = (uint16_t *)Alloc(allocator, transform_matrix_size);
    transform->transform_offset = (uint16_t *)Alloc(allocator, transform_offset_size);
    transform->transform_scale = (uint16_t *)Alloc(allocator, transform_scale_size);
...
CODEC_ERROR AllocateComponentPermutation(ALLOCATOR *allocator, COMPONENT_PERMUTATION *permutation, int component_count)
{
    ...
    permutation->permutation_array = (uint16_t *)Alloc(allocator, permutation_array_size);
...
CODEC_ERROR ReleaseComponentTransform(ALLOCATOR *allocator, COMPONENT_TRANSFORM *transform)
{
    if (transform != NULL)
    {
        Free(allocator, transform->transform_matrix);
        Free(allocator, transform->transform_offset);
        Free(allocator, transform->transform_scale);
...
CODEC_ERROR ReleaseComponentPermutation(ALLOCATOR *allocator, COMPONENT_PERMUTATION *permutation)
{
    if (permutation != NULL)
    {
        Free(allocator, permutation->permutation_array);
...
CODEC_ERROR WriteComponentTransform(COMPONENT_TRANSFORM *transform, BITSTREAM *stream)
{
    if (transform != NULL)
    {
        ...
        const int chunk_payload_padding = sizeof(SEGMENT) - (chunk_payload_size % sizeof(SEGMENT));
        ...
        for (i = 0; i < chunk_payload_padding; i++) {
            PutBits(stream, 0, 8);
        }
